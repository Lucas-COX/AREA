...
func NotionLogin(w http.ResponseWriter, r *http.Request) {
	...
	res.Url = services.Notion.Authenticate(string(callbackUrl), user.ID)

	lib.SendJson(w, res)
}

func NotionCallback(w http.ResponseWriter, r *http.Request) {
	...
	url, err := services.Notion.AuthenticateCallback(state, code)
	if err != nil {
...
func NotionLogout(w http.ResponseWriter, r *http.Request) {
